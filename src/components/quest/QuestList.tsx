...
export default function QuestList({
  ...
  // Derive level sections from quests + ensure current level always shows
  const levelNums = new Set<number>();
  levelNums.add(currentLevel);
  for (const q of quests) levelNums.add(q.level_num);
  const sortedLevelNums = Array.from(levelNums).sort((a, b) => a - b);
  ...
  return (
    <div className="space-y-2">
      {sortedLevelNums.map((levelNum) => {
        ...
        const pendingCount = levelQuests.filter(
          (q) => !q.is_completed || q.is_repeatable
        ).length;
        ...
        return (
          <div
            ...
            className={`border rounded-xl overflow-hidden ${
              isLocked ? "border-warm-100 opacity-60" : "border-warm-200"
            }`}
          >
            <button
